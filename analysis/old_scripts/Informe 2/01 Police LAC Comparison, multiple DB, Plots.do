/*******************************************************************************
	...
	Title:		01	Police LAC Comparison, multiple DB, Plots
	Author:		Lucas García
	Date:		March 07, 2023
	Version:	Stata 17

	Summary:	This dofile plots Police trust trends for LAC and CHL using 2 
...
*-01		Defining temporal file to fill and plot
tempname lgc
tempfile police
postfile `lgc' str10 source double(mean year chile) using `police', replace

*00		Opening 2020 dataset & cleaning
use "$usedata/latinobarometro", clear

*01		Collapse Annualy
collapse (mean) d_police, by(year chile)

*02		Post in temporal file
forvalues v = 1/12{
	post `lgc' ("Latinobarometro") (d_police[`v']) (year[`v']) (chile[`v'])
}	
...
collapse (mean) police, by(year chile)

*02		Post in temporal file
forvalues v = 1/10{
	post `lgc' ("LAPOP") (police[`v']) (year[`v']) (chile[`v'])
}
...
graph export "$graphs/Police trust, Chile y LAC, multiple DB.pdf", replace