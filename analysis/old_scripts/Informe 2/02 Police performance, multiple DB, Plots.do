/*******************************************************************************
	...
	Title:		02	Police performance, multiple DB, Plots
	Author:		Lucas García
	Date:		March 07, 2023
	Version:	Stata 17

	Summary:	This dofile plots Police performance trends using 2 different data 
...
*03		Collapse by year
collapse (mean) Carabineros, by (year)

*04		Post in temporal file
forvalues v = 1/9{
	post `lgc' ("CADEM") (Carabineros[`v']) (year[`v'])
}
...
g year=year(date)
destring NotaCarabineros, replace dpcomma

*Collapse Year level
collapse (mean) NotaCarabineros, by(year)

*02		Post in temporal file
forvalues v = 1/11{
	post `lgc' ("PC") (NotaCarabineros[`v']) (year[`v'])
}
...
graph export "$graphs/Police performance, multiple DB.pdf", replace