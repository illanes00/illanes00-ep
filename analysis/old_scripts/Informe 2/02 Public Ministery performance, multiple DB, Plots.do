/*******************************************************************************
	...
	Title:		02	Public Ministery performance, multiple DB, Plots
	Author:		Lucas García
	Date:		March 07, 2023
	Version:	Stata 17

	Summary:	This dofile plots Public Ministery performance trends using 2 different data 
...
*03		Collapse by year
collapse (mean) Fiscalía, by (year)

*04		Post in temporal file
forvalues v = 1/9{
	post `lgc' ("CADEM") (Fiscalía[`v']) (year[`v'])
}
...
g year=year(date)
destring Fiscales, replace dpcomma

*Collapse Year level
collapse (mean) Fiscales, by(year)

*02		Post in temporal file
forvalues v = 1/11{
	post `lgc' ("PC") (Fiscales[`v']) (year[`v'])
}
...
graph export "$graphs/Public Ministery performance, multiple DB.pdf", replace